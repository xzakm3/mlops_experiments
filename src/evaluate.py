...
def get_overlay_image(img_fpath, mask_true, mask_pred):
    ...
    overlay_img_true = Image.blend(img_pil.convert("RGBA"), paint_mask(mask_true).convert("RGBA"), 0.5)
...
def evaluate():
    ...
    test_mask_fpaths = [get_mask_path(fpath, Path("data") / "test_data") for fpath in test_img_fpaths]
    ...
    with Live("results/evaluate") as live:
        ...
        for ii in range(len(masks_true)):
            ...
            mask_pred = np.array(Image.fromarray(mask_pred).resize((mask_true.shape[1], mask_true.shape[0])), dtype=int)
            ...
            if ii < params.evaluate.n_samples_to_save:
                stacked_image = get_overlay_image(test_img_fpaths[ii], mask_true, mask_pred)
