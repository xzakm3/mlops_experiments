...
def train():
    ...
    model_names = [
        ...
        if not name.startswith("_") and name.islower() and name not in ("all", "tvm", "unet", "xresnet")
    ]
    ...
    with Live("results/train") as live:
        learn = unet_learner(data_loader, arch=getattr(models, params.train.arch), metrics=DiceMulti)
